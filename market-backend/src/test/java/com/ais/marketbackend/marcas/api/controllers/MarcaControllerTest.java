...
class MarcaControllerTest {
    ...
    void setUp() {
        ...
        MarcaApiMapper mapper = resumen -> MarcaResponse.builder().id(resumen.id()).nombre(resumen.nombre()).build();
    ...
    void listarDevuelveLasMarcas() throws Exception {
        when(marcaService.listar()).thenReturn(List.of(new MarcaResumen(1L, "Nestlé")));
    ...
    void crearDevuelve201() throws Exception {
        when(marcaService.crear(anyString())).thenReturn(new MarcaResumen(2L, "Coca-Cola"));
