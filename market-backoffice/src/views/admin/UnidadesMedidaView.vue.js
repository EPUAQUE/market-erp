...
const filtered = computed(() => {
    ...
    if (!term)
        return items.value;
    return items.value.filter((u) => u.nombre.toLowerCase().includes(term) || u.abreviacion.toLowerCase().includes(term));
});
function abrirCrear() {
...
(__VLS_ctx.search);
if (__VLS_ctx.permissions.can('UNIDADES_MEDIDA_CREAR')) {
    ...
    (__VLS_ctx.showForm ? 'Cancelar' : 'Nueva unidad');
}
if (__VLS_ctx.showForm) {
    ...
    __VLS_asFunctionalElement(__VLS_intrinsicElements.input)({
        value: (__VLS_ctx.form.abreviacion),
        type: "text",
        required: true,
        ...{ class: "mk-input w-full rounded border border-mk-border bg-transparent px-3 py-2" },
    });
...
for (const [unidad] of __VLS_getVForSourceType((__VLS_ctx.filtered))) {
    __VLS_asFunctionalElement(__VLS_intrinsicElements.tr, __VLS_intrinsicElements.tr)({
        key: (unidad.id),
    ...
    if (__VLS_ctx.permissions.can('UNIDADES_MEDIDA_EDITAR')) {
        __VLS_asFunctionalElement(__VLS_intrinsicElements.button, __VLS_intrinsicElements.button)({
            ...{ onClick: (...[$event]) => {
                    if (!(__VLS_ctx.permissions.can('UNIDADES_MEDIDA_EDITAR')))
                        return;
                    __VLS_ctx.abrirEditar(unidad);
                } },
            type: "button",
